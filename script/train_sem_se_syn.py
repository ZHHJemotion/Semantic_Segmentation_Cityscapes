...
def train(args):
    ...
    model = DataParallelModel(model, device_ids=args.device_ids).cuda()  # multi-gpu
    ...
    se_loss = SemanticEncodingLoss(num_classes=19, ignore_label=250, alpha=0.50).cuda()
    se_loss_parallel = DataParallelCriterion(se_loss, device_ids=args.device_ids)
    ...
    for epoch in np.arange(args.start_epoch-1, args.num_epochs):
        ...
        for train_i, (images, gt_masks) in zip(range(start_iter, num_batches), tra_loader):
            ...
            train_se_loss = se_loss_parallel(predicts=se, enc_cls_target=se_labels,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device_ids', nargs='?', type=list, default=[0,1,2],
                        help='The ids of device')
    ...
    parser.add_argument('--num_epochs', nargs='?', type=int, default=200,
    ...
    parser.add_argument('--pre_trained', nargs='?', type=str, default='resnet50_imagenet.pth',
                        help='Path to pre-trained  model to init from | None by  default')
    parser.add_argument('--model_details', nargs='?', type=str, default='parallel_resnet50',
