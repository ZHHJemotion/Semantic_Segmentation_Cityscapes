...
from .attention_resnet import resnet50, resnet101
...
class DANet(nn.Module):
    ...
    def forward(self, x):
        imsize = x.size()[2:]
        _, _, _, c4, c5 = self.backbone(x)

        x = self.head(c5)
        x = list(x)
        # x[0] = F.upsample(x[0], imsize)
        # x[1] = F.upsample(x[1], imsize)
        # x[2] = F.upsample(x[2], imsize)

        x[0] = F.interpolate(x[0], imsize)
        x[1] = F.interpolate(x[1], imsize)
        x[2] = F.interpolate(x[2], imsize)
