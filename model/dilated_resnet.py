...
__all__ = ['ResNet', 'dilated_resnet18', 'dilated_resnet34', 'dilated_resnet50', 'dilated_resnet101',
           'dilated_resnet152', 'BasicBlock', 'Bottleneck']
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        c1 = self.conv1(x)      # 1/2   --> (N, 64, 256, 256)
        c1 = self.bn1(c1)
        c1 = self.relu(c1)
        c2 = self.maxpool(c1)

        c2 = self.layer1(c2)    # 1/4   --> (N, 256, 128, 128)
        c3 = self.layer2(c2)    # 1/8   --> (N, 512, 56, 112)
        c4 = self.layer3(c3)    # 1/8   --> (N, 1024, 56, 112)
        c5 = self.layer4(c4)    # 1/8   --> (N, 2048, 56, 112)
